export default function Resume() {
    return (
        <div id="resume" className="section-resume style-1 pb-0 spacing-5 section">
            <div className="heading-section mb_47">
                <div className="tag-heading text-uppercase text-label font-3 letter-spacing-1 mb_30">
                    Resume
                </div>
                <h3 className="text_white fw-5 split-text effect-blur-fade">
                    Experience
                </h3>
            </div>

            <div className="effect-line-hover">
                <div className="wrap-education-item area-effect scrolling-effect effectTop">
                    <span className="point"></span>
                    <div className="education-item">
                        <div className="content">
                            <h5 className="font-4 mb_4">
                                <a href="#" className="link">
                                    AI Developer
                                </a>
                            </h5>
                            <span className="text-body-1 font-3">Google Inc.</span>
                        </div>
                        <div className="date text-caption-1 text_white font-3">
                            2020 - Present
                        </div>
                        <div className="item-shape spotlight">
                            <img
                                src="assets/images/item/small-comet.webp"
                                alt="item"
                                loading="lazy"
                            />
                        </div>
                    </div>
                </div>
                <div className="wrap-education-item area-effect scrolling-effect effectTop">
                    <span className="point"></span>
                    <div className="education-item">
                        <div className="content">
                            <h5 className="font-4 mb_4">
                                <a href="#contact" className="link">
                                    Machine Learning Engineer
                                </a>
                            </h5>
                            <span className="text-body-1 font-3">Microsoft Inc.</span>
                        </div>
                        <div className="date text-caption-1 text_white font-3">
                            2018 - 2020
                        </div>
                        <div className="item-shape spotlight">
                            <img
                                src="assets/images/item/small-comet.webp"
                                alt="item"
                                loading="lazy"
                            />
                        </div>
                    </div>
                </div>
            </div>

            <div className="spacing-4 ">
                <div className="heading-section mb_47">
                    <h3 className="text_white fw-5 split-text effect-blur-fade">
                        Education
                    </h3>
                </div>
                <div className="effect-line-hover">
                    <div className="wrap-education-item area-effect scrolling-effect effectTop">
                        <span className="point"></span>
                        <div className="education-item">
                            <div className="content">
                                <h5 className="font-4 mb_4">
                                    <a href="#contact" className="link">
                                        AI Developer
                                    </a>
                                </h5>
                                <span className="text-body-1 font-3">Google Inc.</span>
                            </div>
                            <div className="date text-caption-1 text_white font-3">
                                2020 - Present
                            </div>
                            <div className="item-shape spotlight">
                                <img
                                    src="assets/images/item/small-comet.webp"
                                    alt="item"
                                    loading="lazy"
                                />
                            </div>
                        </div>
                    </div>
                    <div className="wrap-education-item area-effect scrolling-effect effectTop">
                        <span className="point"></span>
                        <div className="education-item">
                            <div className="content">
                                <h5 className="font-4 mb_4">
                                    <a href="#contact" className="link">
                                        Machine Learning Engineer
                                    </a>
                                </h5>
                                <span className="text-body-1 font-3">Microsoft Inc.</span>
                            </div>
                            <div className="date text-caption-1 text_white font-3">
                                2018 - 2020
                            </div>
                            <div className="item-shape spotlight">
                                <img
                                    src="assets/images/item/small-comet.webp"
                                    alt="item"
                                    loading="lazy"
                                />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}
